...
export const PRODUCT_EXPERIMENT_PROMPT_DEFAULT_TEMPLATES: ProductExperimentPromptTemplate[] = [
  ...
  },
  {
    ...
    instructions_md: [
      ...
      '',
      'Response format:',
      '- Start with a "Required data preflight" markdown table before any diagnosis.',
      '- Preflight rows must include: sales_trend_daily, ads_baseline.sp.targets, ads_baseline.sp.campaigns, placement modifiers, STIS, STIR, sales_adjustments_daily, placement_modifier_change_log.',
      '- Preflight table columns must be: Artifact, Status (pass/fail), Notes.',
      '- Use markdown tables for KPI comparisons (before/after, placement breakdown, target breakdown).',
      '- Use markdown tables for recommended changes (what, why, expected impact).',
      '- When referencing entities, always include id + name: campaign_id + campaign_name_raw, ad_group_id + ad_group_name_raw, and target_id + targeting_raw (plus campaign_id/ad_group_id).',
      '- KPI scope rule: reference ads_kpi_scope_glossary for STIS/STIR/TOS IS interpretation.',
      '- Never average STIS/STIR/TOS IS; treat them as diagnostics only.',
      '- Placement modifier scope rule: placement modifiers are campaign-level and affect all targets.',
      '- If a campaign has multiple targets, explicitly summarize tradeoffs so modifier changes do not harm other high-performing targets.',
      '- Start analysis with unit economics first (margin/TACOS/profit trend) when present.',
      '',
      'Workflow:',
