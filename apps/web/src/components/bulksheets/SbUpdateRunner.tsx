...
import { buildSbUpdateActions, type SbUpdateRowInput } from '@/lib/bulksheets/actionBuilders';
...
const defaultRow = (): SbUpdateRowInput => ({
...
const buildRunId = () => {
  ...
  return `ui-sb-update-${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(
    now.getDate()
...
export default function SbUpdateRunner(props: {
  action: (prevState: SbUpdateState, formData: FormData) => Promise<SbUpdateState>;
  ...
  const [rows, setRows] = useState<SbUpdateRowInput[]>([defaultRow()]);
  ...
  const builderState = useMemo(() => {
    try {
      const actions = buildSbUpdateActions(rows);
  ...
  const updateRow = (index: number, field: keyof SbUpdateRowInput, value: string) => {
    setRows((prev) =>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2 md:col-span-2">
              ...
              <input
                ...
                placeholder="/path/to/sb-update-template.xlsx"
                required
              />
            </div>
            <div className="space-y-2 md:col-span-2">
                ...
                <div key={`row-${index}`} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        value={row.placement_raw ?? ''}
                        onChange={(event) => updateRow(index, 'placement_raw', event.target.value)}
                        className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                      />
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        value={row.new_default_bid ?? ''}
                        onChange={(event) => updateRow(index, 'new_default_bid', event.target.value)}
                        className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm"
                        type="number"
                        step="0.01"
                      />
