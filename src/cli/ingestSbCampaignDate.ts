...
import { resolveDateFolder, getSbCampaignXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sb:campaign:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const xlsxPath = getSbCampaignXlsx(dateFolder);
  const exportedAt = exportedAtArg ?? exportedAtFromFolder(dateFolder) ?? undefined;

  const result = await ingestSbCampaignRaw(xlsxPath, accountId, exportedAt);
