...
import { resolveDateFolder, getSdPurchasedProductXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sd:purchased:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const xlsxPath = getSdPurchasedProductXlsx(dateFolder);
  const exportedAt = exportedAtArg ?? exportedAtFromFolder(dateFolder) ?? undefined;

  const result = await ingestSdPurchasedProductRaw(xlsxPath, accountId, exportedAt);
