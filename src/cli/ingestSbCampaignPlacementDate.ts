...
import { resolveDateFolder, getSbCampaignPlacementXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sb:campaign:placement:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const xlsxPath = getSbCampaignPlacementXlsx(dateFolder);
  const exportedAt = exportedAtArg ?? exportedAtFromFolder(dateFolder) ?? undefined;

  const result = await ingestSbCampaignPlacementRaw(xlsxPath, accountId, exportedAt);
