...
import { resolveDateFolder, getSdCampaignXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sd:campaign:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const xlsxPath = getSdCampaignXlsx(dateFolder);
  const exportedAt = exportedAtArg ?? exportedAtFromFolder(dateFolder) ?? undefined;

  const result = await ingestSdCampaignRaw(xlsxPath, accountId, exportedAt);
