...
function usage() {
  console.log(
    "Usage: npm run ingest:sqp:weekly -- --account-id <id> --marketplace <marketplace> <csv> [--exported-at ISO]"
  );
}
...
async function main() {
  ...
  const result = await ingestSqpWeeklyRaw(csvPath, accountId, marketplace, exportedAtArg);
