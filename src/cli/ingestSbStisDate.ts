...
function usage() {
  console.log(
    "Usage: npm run ingest:sb:stis:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const csvPath = getSbStisCsv(dateFolder);
  const exportedAt = exportedAtArg ?? exportedAtFromFolder(dateFolder) ?? undefined;

  const result = await ingestSbStisRaw(csvPath, accountId, exportedAt);
