...
import { resolveDateFolder, getSdTargetingXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sd:targeting:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const xlsxPath = getSdTargetingXlsx(dateFolder);
  const exportedAt = exportedAtArg ?? exportedAtFromFolder(dateFolder) ?? undefined;

  const result = await ingestSdTargetingRaw(xlsxPath, accountId, exportedAt);
