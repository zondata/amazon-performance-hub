...
import { resolveDateFolder, getSbKeywordXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sb:keyword:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const xlsxPath = getSbKeywordXlsx(dateFolder);
  const exportedAt = exportedAtArg ?? exportedAtFromFolder(dateFolder) ?? undefined;

  const result = await ingestSbKeywordRaw(xlsxPath, accountId, exportedAt);
