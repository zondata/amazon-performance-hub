...
import { resolveDateFolder, getSpTargetingXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sp:targeting:date -- --account-id US <YYYY-MM-DD or folder>"
  );
}
...
async function main() {
  ...
  const result = await ingestSpTargetingRaw(xlsxPath, accountId, exportedAt);
