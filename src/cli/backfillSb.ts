import { backfillSb } from "../pipeline/backfillSb";

function usage() {
  console.log(
    "Usage: npm run pipeline:backfill:sb -- --account-id US --root <path> --from YYYY-MM-DD --to YYYY-MM-DD [--concurrency N] [--dry-run] [--continue-on-error]"
  );
}
...
async function main() {
  ...
  const summaries = await backfillSb({
    accountId,
    root,
    from,
    to,
