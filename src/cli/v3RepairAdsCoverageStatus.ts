...
const REPAIR_SPECS: CoverageSpec[] = [
  ...
  },
  {
    ...
    successNote:
      'SP Search Term Daily imported successfully for the latest available period.',
    failedLabel: 'SP Search Term Daily',
  },
];
...
const readLatestSalesSuccess = async (
  pool: Pool,
  args: { accountId: string; marketplace: string }
): Promise<{ syncRunId: string | null; finishedAt: string | null }> => {
  const result = await pool.query(
    `
      select
      ...
      where account_id = $1
        and marketplace = $2
        and source_type = 'sp_api'
        and source_name = 'sales'
        and status = 'succeeded'
      order by finished_at desc nulls last, requested_at desc nulls last
      limit 1
    `,
    [args.accountId, args.marketplace]
  );
  ...
  return {
    syncRunId:
      typeof result.rows[0]?.sync_run_id === 'string'
        ? result.rows[0].sync_run_id
        : null,
    finishedAt:
      typeof result.rows[0]?.finished_at === 'string'
...
export const deriveRepairedCoverageRow = (args: {
  ...
  fallbackSuccessfulRunAt: string | null;
  nowIso: string;
  ...
  const { spec, stats, latestRequest, fallbackSuccessfulRunAt } = args;
  ...
  if (latestRequest?.status === 'imported') {
    ...
    lastSuccessfulRunAt =
      ...
      fallbackSuccessfulRunAt;
    notes =
      spec.sourceType === 'sp_api_sales_traffic_daily'
        ? 'Sales & Traffic coverage was repaired from existing imported fact data.'
...
export const repairAdsCoverageStatus = async (
  ...
  const [adsLastSyncRunId, latestSalesSuccess] = await Promise.all([
    readLatestSyncRunId(pool, args),
    readLatestSalesSuccess(pool, args),
  ]);
  ...
  for (const spec of REPAIR_SPECS) {
    ...
    const row = deriveRepairedCoverageRow({
      ...
      lastSyncRunId:
        spec.sourceType === 'sp_api_sales_traffic_daily'
          ? latestSalesSuccess.syncRunId
          : adsLastSyncRunId,
      fallbackSuccessfulRunAt:
        spec.sourceType === 'sp_api_sales_traffic_daily'
          ? latestSalesSuccess.finishedAt
          : null,
      nowIso: finishedAt,
    });
