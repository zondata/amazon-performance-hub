...
import { resolveDateFolder, getSpAdvertisedProductXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sp:advertised:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const xlsxPath = getSpAdvertisedProductXlsx(dateFolder);
