...
import { resolveDateFolder, getSdMatchedTargetXlsx } from "../fs/reportLocator";

function usage() {
  console.log(
    "Usage: npm run ingest:sd:matched:date -- --account-id <id> <date-folder-or-date> [--exported-at ISO]\n" +
...
async function main() {
  ...
  const xlsxPath = getSdMatchedTargetXlsx(dateFolder);
  const exportedAt = exportedAtArg ?? exportedAtFromFolder(dateFolder) ?? undefined;

  const result = await ingestSdMatchedTargetRaw(xlsxPath, accountId, exportedAt);
