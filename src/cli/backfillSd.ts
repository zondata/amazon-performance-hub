...
function usage() {
  console.log(
    "Usage: npm run pipeline:backfill:sd -- --account-id US --root <path> --from YYYY-MM-DD --to YYYY-MM-DD [--concurrency N] [--dry-run] [--continue-on-error]"
  );
}
...
async function main() {
  ...
  const summaries = await backfillSd({
    accountId,
    root,
    from,
    to,
