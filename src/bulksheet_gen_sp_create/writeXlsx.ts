...
export function writeSpBulkCreateXlsx(params: {
  templatePath: string;
  outDir: string;
  rows: UploadRow[];
  ...
  const { templatePath, outDir, rows, requiredHeadersBySheet, writeUpload } = params;
  ...
  if (writeUpload) {
    uploadPath = path.join(outDir, "upload_strict.xlsx");
