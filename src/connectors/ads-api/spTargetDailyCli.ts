...
const readInteger = (value: string | undefined, label: string): number => {
  ...
  if (!Number.isInteger(parsed) || parsed <= 0) {
    throw new AdsApiSpTargetDailyError('invalid_date', `${label} must be a positive integer.`);
  }
  return parsed;
};
...
const createPendingRequestStore = (
  pool: Pool
): SpTargetDailyPendingRequestStore => ({
...
const loadExistingTargetArtifact = (args: {
  ...
  if (
    !fs.existsSync(ADS_API_SP_TARGET_DAILY_RAW_ARTIFACT_PATH) ||
    !fs.existsSync(ADS_API_SP_TARGET_DAILY_NORMALIZED_ARTIFACT_PATH)
  ) {
    return null;
  }
  ...
  try {
    const parsed = JSON.parse(
      fs.readFileSync(ADS_API_SP_TARGET_DAILY_NORMALIZED_ARTIFACT_PATH, 'utf8')
    ) as Partial<AdsApiSpTargetDailyNormalizedArtifact>;

    if (
      parsed.schemaVersion !== 'ads-api-sp-target-daily-normalized/v1' ||
      ...
      !Array.isArray(parsed.normalizedTargetRows)
    ) {
      return null;
    }

    return {
      normalizedArtifact: parsed as AdsApiSpTargetDailyNormalizedArtifact,
      rawArtifactPath: ADS_API_SP_TARGET_DAILY_RAW_ARTIFACT_PATH,
      normalizedArtifactPath: ADS_API_SP_TARGET_DAILY_NORMALIZED_ARTIFACT_PATH,
    };
  } catch {
    return null;
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const existingArtifact = loadExistingTargetArtifact({
      config,
      dateRange,
    });
    ...
    let result;
    try {
      result = await runSpTargetDailyPull({
        config,
      ...
      if (
        error instanceof AdsApiSpTargetDailyError &&
        (error.code === 'report_timeout' || error.code === 'pending_timeout')
      ) {
        const timedOutArtifact = loadExistingTargetArtifact({
          config,
          dateRange,
        });
    ...
    if (error instanceof AdsApiConfigError) {
      ...
      console.error(`Amazon Ads target daily error code: ${error.code}`);
