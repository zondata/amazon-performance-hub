...
const USAGE =
  'Usage: npm run adsapi:pull-sp-search-term-daily -- --start-date YYYY-MM-DD --end-date YYYY-MM-DD [--resume-pending] [--resume-pending-only] [--max-attempts N] [--poll-interval-ms N]';

const readInteger = (value: string | undefined, label: string): number => {
  const parsed = Number.parseInt(value ?? '', 10);
  if (!Number.isInteger(parsed) || parsed <= 0) {
    throw new AdsApiSpSearchTermDailyError(
      'invalid_date',
...
const parseArgs = (argv: string[]): CliArgs => {
  ...
  if (!startDate || !endDate) {
    throw new AdsApiSpSearchTermDailyError('invalid_date', USAGE);
  }
...
const createPendingRequestStore = (
  pool: Pool
): SpSearchTermDailyPendingRequestStore => ({
...
export const buildSpSearchTermDailySuccessLines = (args: {
  ...
  'Amazon Ads search term daily pull succeeded.',
...
const loadExistingSearchTermArtifact = (args: {
  ...
  if (
    !fs.existsSync(ADS_API_SP_SEARCH_TERM_DAILY_RAW_ARTIFACT_PATH) ||
    !fs.existsSync(ADS_API_SP_SEARCH_TERM_DAILY_NORMALIZED_ARTIFACT_PATH)
  ) {
    return null;
  }
  ...
  try {
    const parsed = JSON.parse(
      fs.readFileSync(ADS_API_SP_SEARCH_TERM_DAILY_NORMALIZED_ARTIFACT_PATH, 'utf8')
    ) as Partial<AdsApiSpSearchTermDailyNormalizedArtifact>;

    if (
      parsed.schemaVersion !== 'ads-api-sp-search-term-daily-normalized/v1' ||
      ...
      !Array.isArray(parsed.normalizedSearchTermRows)
    ) {
      return null;
    }

    return {
      normalizedArtifact: parsed as AdsApiSpSearchTermDailyNormalizedArtifact,
      rawArtifactPath: ADS_API_SP_SEARCH_TERM_DAILY_RAW_ARTIFACT_PATH,
      normalizedArtifactPath: ADS_API_SP_SEARCH_TERM_DAILY_NORMALIZED_ARTIFACT_PATH,
    };
  } catch {
    return null;
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (databaseUrl) {
      ...
      throw new AdsApiSpSearchTermDailyError(
        'pending_timeout',
        'DATABASE_URL is required to resume a pending Ads SP search term report.'
      );
    }
    const pendingStore = pool ? createPendingRequestStore(pool) : null;
    const existingArtifact = loadExistingSearchTermArtifact({
      config,
      dateRange,
    });

    if (existingArtifact) {
      for (const line of buildSpSearchTermDailySuccessLines({
    ...
    let result;
    try {
      result = await runSpSearchTermDailyPull({
        config,
        ...
        diagnosticPath: ADS_API_SP_SEARCH_TERM_DAILY_DIAGNOSTIC_ARTIFACT_PATH,
      });
    } catch (error) {
      if (
        error instanceof AdsApiSpSearchTermDailyError &&
        (error.code === 'report_timeout' || error.code === 'pending_timeout')
      ) {
        const timedOutArtifact = loadExistingSearchTermArtifact({
          config,
          dateRange,
        });
    ...
    } else if (error instanceof AdsApiSpSearchTermDailyError) {
      console.error(`Amazon Ads search term daily error code: ${error.code}`);
      console.error(`Amazon Ads search term daily error: ${error.message}`);
    } else if (error instanceof Error) {
      console.error(`Amazon Ads search term daily pull failed: ${error.message}`);
    } else {
      console.error(
        'Amazon Ads search term daily pull failed due to an unknown error.'
      );
    }
