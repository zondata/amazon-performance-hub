...
import {
  ...
  DEFAULT_SP_ADVERTISED_PRODUCT_DAILY_MAX_ATTEMPTS,
  DEFAULT_SP_ADVERTISED_PRODUCT_DAILY_POLL_INTERVAL_MS,
  ...
  type SpAdvertisedProductDailyPendingRequestStore,
} from './index';
...
const USAGE =
  'Usage: npm run adsapi:pull-sp-advertised-product-daily -- --start-date YYYY-MM-DD --end-date YYYY-MM-DD [--resume-pending] [--resume-pending-only] [--max-attempts N] [--poll-interval-ms N]';

const readInteger = (value: string | undefined, label: string): number => {
  const parsed = Number.parseInt(value ?? '', 10);
  if (!Number.isInteger(parsed) || parsed <= 0) {
    throw new AdsApiSpAdvertisedProductDailyError(
      'invalid_date',
...
const parseArgs = (argv: string[]): CliArgs => {
  ...
  if (!startDate || !endDate) {
    throw new AdsApiSpAdvertisedProductDailyError('invalid_date', USAGE);
  }
...
const createPendingRequestStore = (
  pool: Pool
): SpAdvertisedProductDailyPendingRequestStore => ({
...
export const buildSpAdvertisedProductDailySuccessLines = (args: {
  ...
  'Amazon Ads advertised product daily pull succeeded.',
...
const loadExistingAdvertisedProductArtifact = (args: {
  ...
  if (
    !fs.existsSync(ADS_API_SP_ADVERTISED_PRODUCT_DAILY_RAW_ARTIFACT_PATH) ||
    !fs.existsSync(ADS_API_SP_ADVERTISED_PRODUCT_DAILY_NORMALIZED_ARTIFACT_PATH)
  ) {
    return null;
  }
  ...
  try {
    const parsed = JSON.parse(
      fs.readFileSync(ADS_API_SP_ADVERTISED_PRODUCT_DAILY_NORMALIZED_ARTIFACT_PATH, 'utf8')
    ) as Partial<AdsApiSpAdvertisedProductDailyNormalizedArtifact>;

    if (
      parsed.schemaVersion !== 'ads-api-sp-advertised-product-daily-normalized/v1' ||
      ...
      !Array.isArray(parsed.normalizedAdvertisedProductRows)
    ) {
      return null;
    }

    return {
      normalizedArtifact: parsed as AdsApiSpAdvertisedProductDailyNormalizedArtifact,
      rawArtifactPath: ADS_API_SP_ADVERTISED_PRODUCT_DAILY_RAW_ARTIFACT_PATH,
      normalizedArtifactPath: ADS_API_SP_ADVERTISED_PRODUCT_DAILY_NORMALIZED_ARTIFACT_PATH,
    };
  } catch {
    return null;
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (databaseUrl) {
      ...
      throw new AdsApiSpAdvertisedProductDailyError(
        'pending_timeout',
        'DATABASE_URL is required to resume a pending Ads SP advertised product report.'
      );
    }
    const pendingStore = pool ? createPendingRequestStore(pool) : null;
    const existingArtifact = loadExistingAdvertisedProductArtifact({
      config,
      dateRange,
    });

    if (existingArtifact) {
      for (const line of buildSpAdvertisedProductDailySuccessLines({
    ...
    let result;
    try {
      result = await runSpAdvertisedProductDailyPull({
        config,
        ...
        diagnosticPath: ADS_API_SP_ADVERTISED_PRODUCT_DAILY_DIAGNOSTIC_ARTIFACT_PATH,
      });
    } catch (error) {
      if (
        error instanceof AdsApiSpAdvertisedProductDailyError &&
        (error.code === 'report_timeout' || error.code === 'pending_timeout')
      ) {
        const timedOutArtifact = loadExistingAdvertisedProductArtifact({
          config,
          dateRange,
        });
    ...
    } else if (error instanceof AdsApiSpAdvertisedProductDailyError) {
      console.error(`Amazon Ads advertised product daily error code: ${error.code}`);
      console.error(`Amazon Ads advertised product daily error: ${error.message}`);
    } else if (error instanceof Error) {
      console.error(`Amazon Ads advertised product daily pull failed: ${error.message}`);
    } else {
      console.error(
        'Amazon Ads advertised product daily pull failed due to an unknown error.'
      );
    }
