...
export type SdPurchasedProductRow = {
  date: string;
...
export type SdPurchasedProductParseResult = {
  rows: SdPurchasedProductRow[];
...
const HEADER_ALIASES: Record<string, string[]> = {
  ...
  purchased_sku_raw: ["purchased sku", "purchased product sku"],
  purchased_asin_raw: ["purchased asin", "purchased product asin", "purchased product"],
...
export function parseSdPurchasedProductReport(input: string): SdPurchasedProductParseResult {
  ...
  const rows: SdPurchasedProductRow[] = [];
  ...
  for (let i = 1; i < matrix.length; i += 1) {
    ...
    if (!purchasedSkuRaw && !purchasedAsinRaw) continue;
    ...
    rows.push({
      ...
      purchased_sku_norm: purchasedSkuRaw ? normText(purchasedSkuRaw) : null,
      purchased_asin_raw: purchasedAsinRaw || null,
      purchased_asin_norm: purchasedAsinRaw ? normText(purchasedAsinRaw) : null,
