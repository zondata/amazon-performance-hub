...
export type SbCampaignPlacementRow = {
  date: string;
...
export type SbCampaignPlacementParseResult = {
  rows: SbCampaignPlacementRow[];
...
export function parseSbCampaignPlacementReport(input: string): SbCampaignPlacementParseResult {
  ...
  const rows: SbCampaignPlacementRow[] = [];
  ...
  for (let i = 1; i < matrix.length; i += 1) {
    ...
    rows.push({
      ...
      placement_raw_norm: normText(placementRaw),
