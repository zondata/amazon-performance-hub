...
export type SpPlacementRow = {
  ...
  placement_raw_norm: string; // ✅ added
...
export type SpPlacementParseResult = {
  rows: SpPlacementRow[];
...
const HEADER_ALIASES: Record<string, string[]> = {
  ...
  bidding_strategy: ["bidding strategy", "bidding strategy type"],
  placement_raw: ["placement", "placement type"],
...
function normalizePlacementCode(raw: string): string {
  ...
  if (norm.includes("top of search")) return "TOS";
  if (norm.includes("rest of search")) return "ROS";
  if (norm.includes("product pages")) return "PP";
  if (norm.includes("off amazon")) return "OA";
  return "UNKNOWN";
}

export function parseSpPlacementReport(input: string): SpPlacementParseResult {
  const isFile = fs.existsSync(input);
  const workbook = isFile
    ? XLSX.readFile(input, { dense: true })
  ...
  for (let i = 1; i < matrix.length; i += 1) {
    ...
    const placementRaw = String(
      headerMap.placement_raw !== undefined ? row[headerMap.placement_raw] ?? "" : ""
    ).trim();
    if (!placementRaw) continue;

    const biddingStrategy =
      headerMap.bidding_strategy !== undefined
        ? String(row[headerMap.bidding_strategy] ?? "").trim()
        : "";
    ...
    rows.push({
      ...
      placement_raw_norm: normText(placementRaw), // ✅ added
      placement_code: normalizePlacementCode(placementRaw),
