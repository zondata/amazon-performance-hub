...
export type SdMatchedTargetRow = {
  date: string;
...
export type SdMatchedTargetParseResult = {
  rows: SdMatchedTargetRow[];
...
const HEADER_ALIASES: Record<string, string[]> = {
  ...
  targeting_raw: ["targeting", "targeting expression", "targeting criteria"],
  matched_target_raw: ["matched target", "matched targeting", "matched target expression"],
...
export function parseSdMatchedTargetReport(input: string): SdMatchedTargetParseResult {
  ...
  for (let i = 1; i < matrix.length; i += 1) {
    ...
    const matchedRaw =
      headerMap.matched_target_raw !== undefined ? row[headerMap.matched_target_raw] ?? "" : "";
    const matchedTargetRaw = String(matchedRaw).trim();
    if (!matchedTargetRaw) continue;
    ...
    rows.push({
      ...
      matched_target_raw: matchedTargetRaw,
      matched_target_norm: normText(matchedTargetRaw),
