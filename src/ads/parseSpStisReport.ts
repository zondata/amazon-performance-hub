...
export function parseSpStisReport(input: string): SpStisParseResult {
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    const nextValue = headerMap.date !== undefined ? row[headerMap.date + 1] ?? "" : "";
    const date = parseDate(String(dateValue ?? ""), String(nextValue ?? ""));
    if (!date) continue;
    ...
    const searchTermRaw = headerMap.customer_search_term_raw !== undefined ? row[headerMap.customer_search_term_raw] ?? "" : "";
    ...
    dataRows.push({
      ...
      search_term_impression_rank: headerMap.search_term_impression_rank !== undefined
        ? parseIntSafe(String(row[headerMap.search_term_impression_rank] ?? ""))
        : null,
      search_term_impression_share: headerMap.search_term_impression_share !== undefined
