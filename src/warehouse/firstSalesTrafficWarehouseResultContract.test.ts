...
const makeTempDir = () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'spapi-warehouse-result-contract-test-')
  );
  tempDirs.push(dir);
  return dir;
};
...
describe('first Sales and Traffic warehouse adapter result contract boundary', () => {
  it('resolves the deterministic warehouse invocation artifact path from report id', async () => {
    ...
    const { warehouseInvocationArtifactPath } =
      await writeWarehouseInvocationArtifact({
  ...
  it('builds the result contract artifact with deterministic target results and lineage', async () => {
    const warehouseReadyDir = makeTempDir();
    const warehouseMappingDir = makeTempDir();
    const warehouseDryRunDir = makeTempDir();
    const warehouseInterfaceDir = makeTempDir();
    ...
    expect(resultContractArtifact).toMatchObject({
    ...
    expect(resultContractArtifact.resultContractPayload.targetResults).toHaveLength(2);
    expect(resultContractArtifact.resultContractPayload.targetResults[0]).toMatchObject({
      sectionName: 'salesAndTrafficByDate',
      targetTableName: 'spapi_sales_and_traffic_by_date_report_rows',
      operationName:
  ...
  it('validates required top-level fields and blocked result flags', async () => {
    const resultContractArtifact =
      buildFirstSalesTrafficWarehouseAdapterResultContract({
        invocationArtifact: {
          warehouseAdapterInvocationVersion:
        ...
        warehouseInvocationArtifactPath: '/tmp/report-report-123.warehouse-invocation.json',
      });
    ...
    expect(() =>
      validateFirstSalesTrafficWarehouseAdapterResultContract({
        resultContractArtifact: {
          ...resultContractArtifact,
          resultContractPayload: {
            ...resultContractArtifact.resultContractPayload,
            targetResults:
              resultContractArtifact.resultContractPayload.targetResults.map(
                (targetResult) => ({
                  ...targetResult,
                  resultState: {
                    ...targetResult.resultState,
    ...
    ).toThrow(/blocked no-write result flags/);
  });

  it('summarizes safely without exposing request or result payload contents', async () => {
    const warehouseReadyDir = makeTempDir();
    const warehouseMappingDir = makeTempDir();
    const warehouseDryRunDir = makeTempDir();
    const warehouseInterfaceDir = makeTempDir();
    ...
    const summary = await runFirstSalesTrafficWarehouseAdapterResultContract({
      warehouseInvocationArtifactPath,
  ...
  it('writes the result contract artifact to the deterministic bounded output path', async () => {
    const warehouseReadyDir = makeTempDir();
    const warehouseMappingDir = makeTempDir();
    const warehouseDryRunDir = makeTempDir();
    const warehouseInterfaceDir = makeTempDir();
    ...
    expect(warehouseResultContractArtifactPath).toBe(
      buildFirstSalesTrafficWarehouseAdapterResultContractPath({
        reportId: 'report-123',
        outputRoot: warehouseResultContractDir,
      })
    );
    expect(fs.existsSync(warehouseResultContractArtifactPath)).toBe(true);
  });

  it('raises typed errors for malformed or mismatched inputs', async () => {
    const warehouseInvocationDir = makeTempDir();
    const malformedArtifactPath = path.join(
      warehouseInvocationDir,
      'report-report-123.warehouse-invocation.json'
    );
    fs.writeFileSync(
    ...
    await expect(
      readFirstSalesTrafficWarehouseAdapterInvocationArtifact({
        warehouseInvocationArtifactPath: malformedArtifactPath,
      })
    ).rejects.toMatchObject({
      name: 'FirstReportWarehouseResultContractError',
