...
import { parseSdPurchasedProductReport } from "../ads/parseSdPurchasedProductReport";
import { chunkArray, hashFileSha256 } from "./utils";
import { retryAsync, isTransientSupabaseError, formatRetryError } from "../lib/retry";

export type SdPurchasedProductIngestResult = {
...
export async function ingestSdPurchasedProductRaw(
  xlsxPath: string,
  accountId: string,
  exportedAtOverride?: string
): Promise<SdPurchasedProductIngestResult> {
  ...
  if (existingUpload?.upload_id) {
    const { count, error: countError } = await client
      .from("sd_purchased_product_daily_raw")
  ...
  const { rows, coverageStart, coverageEnd } = parseSdPurchasedProductReport(xlsxPath);
  ...
  if (!uploadId) {
    const uploadPayload = {
      ...
      source_type: "sd_purchased_product",
  ...
  for (const chunk of chunkArray(rowsToInsert, 500)) {
    const { error } = await client.from("sd_purchased_product_daily_raw").insert(chunk);
    if (error) {
      throw new Error(`Failed inserting sd_purchased_product_daily_raw: ${error.message}`);
    }
  }
