...
export async function ingestSbKeywordRaw(
  xlsxPath: string,
  accountId: string,
  exportedAtOverride?: string
): Promise<SbKeywordIngestResult> {
  ...
  const { rows, coverageStart, coverageEnd } = parseSbKeywordReport(xlsxPath);
  ...
  for (const chunk of chunkArray(rowsToInsert, 500)) {
    const { error } = await client.from("sb_keyword_daily_raw").insert(chunk);
    if (error) throw new Error(`Failed inserting sb_keyword_daily_raw: ${error.message}`);
  }
