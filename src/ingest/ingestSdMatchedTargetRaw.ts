...
export async function ingestSdMatchedTargetRaw(
  xlsxPath: string,
  accountId: string,
  exportedAtOverride?: string
): Promise<SdMatchedTargetIngestResult> {
  ...
  if (existingUpload?.upload_id) {
    const { count, error: countError } = await client
      .from("sd_matched_target_daily_raw")
  ...
  const { rows, coverageStart, coverageEnd } = parseSdMatchedTargetReport(xlsxPath);
  ...
  for (const chunk of chunkArray(rowsToInsert, 500)) {
    const { error } = await client.from("sd_matched_target_daily_raw").insert(chunk);
    if (error) {
      throw new Error(`Failed inserting sd_matched_target_daily_raw: ${error.message}`);
    }
  }
