...
export async function ingestSdTargetingRaw(
  xlsxPath: string,
  accountId: string,
  exportedAtOverride?: string
): Promise<SdTargetingIngestResult> {
  ...
  const { rows, coverageStart, coverageEnd } = parseSdTargetingReport(xlsxPath);
  ...
  for (const chunk of chunkArray(rowsToInsert, 500)) {
    const { error } = await client.from("sd_targeting_daily_raw").insert(chunk);
    if (error) throw new Error(`Failed inserting sd_targeting_daily_raw: ${error.message}`);
  }
