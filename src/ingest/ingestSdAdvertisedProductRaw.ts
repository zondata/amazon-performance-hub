...
import { parseSdAdvertisedProductReport } from "../ads/parseSdAdvertisedProductReport";
import { chunkArray, hashFileSha256 } from "./utils";
import { retryAsync, isTransientSupabaseError, formatRetryError } from "../lib/retry";

export type SdAdvertisedProductIngestResult = {
...
export async function ingestSdAdvertisedProductRaw(
  xlsxPath: string,
  accountId: string,
  exportedAtOverride?: string
): Promise<SdAdvertisedProductIngestResult> {
  ...
  if (existingUpload?.upload_id) {
    const { count, error: countError } = await client
      .from("sd_advertised_product_daily_raw")
  ...
  const { rows, coverageStart, coverageEnd } = parseSdAdvertisedProductReport(xlsxPath);
  ...
  if (!uploadId) {
    const uploadPayload = {
      ...
      source_type: "sd_advertised_product",
  ...
  for (const chunk of chunkArray(rowsToInsert, 500)) {
    const { error } = await client.from("sd_advertised_product_daily_raw").insert(chunk);
    if (error) {
      throw new Error(`Failed inserting sd_advertised_product_daily_raw: ${error.message}`);
    }
  }
