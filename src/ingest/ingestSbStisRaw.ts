...
export async function ingestSbStisRaw(
  csvPath: string,
  accountId: string,
  exportedAtOverride?: string
): Promise<SbStisIngestResult> {
  ...
  const { rows, coverageStart, coverageEnd } = parseSbStisReport(csvPath);
  ...
  for (const chunk of chunkArray(rowsToInsert, 500)) {
    const { error } = await client.from("sb_stis_daily_raw").insert(chunk);
    if (error) throw new Error(`Failed inserting sb_stis_daily_raw: ${error.message}`);
  }
