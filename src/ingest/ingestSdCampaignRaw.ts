...
export async function ingestSdCampaignRaw(
  xlsxPath: string,
  accountId: string,
  exportedAtOverride?: string
): Promise<SdCampaignIngestResult> {
  ...
  const { rows, coverageStart, coverageEnd } = parseSdCampaignReport(xlsxPath);
  ...
  for (const chunk of chunkArray(rowsToInsert, 500)) {
    const { error } = await client.from("sd_campaign_daily_raw").insert(chunk);
    if (error) throw new Error(`Failed inserting sd_campaign_daily_raw: ${error.message}`);
  }
