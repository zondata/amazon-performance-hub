...
export async function ingestSbCampaignRaw(
  xlsxPath: string,
  accountId: string,
  exportedAtOverride?: string
): Promise<SbCampaignIngestResult> {
  ...
  const { rows, coverageStart, coverageEnd } = parseSbCampaignReport(xlsxPath);
  ...
  for (const chunk of chunkArray(rowsToInsert, 500)) {
    const { error } = await client.from("sb_campaign_daily_raw").insert(chunk);
    if (error) throw new Error(`Failed inserting sb_campaign_daily_raw: ${error.message}`);
  }
